...
for scene_name in scene_names:
    ...
    for i in ["Egocentric", "Non-Egocentric"]:
        ...
        os.system(f"python ./Image_datasets/convert_blender360_to_openmvg.py {matches_path}/sfm_data.json {scene_path}/{i}/cameras {output_path}") # Write gt camera parameter to sfm_data.json
        os.system(f"openMVG_main_ComputeFeatures -i {matches_path}/sfm_data_gtpose.json -o {matches_path} -p ULTRA") # Set -p to “ULTRA” to increase the number of points
        os.system(f"openMVG_main_PairGenerator -i {matches_path}/sfm_data_gtpose.json -o {matches_path}/pairs.bin")
        os.system(f"openMVG_main_ComputeMatches -i {matches_path}/sfm_data_gtpose.json -p {matches_path}/pairs.bin -o {matches_path}/matches.putative.bin -r 0.3") # Set -r to “0.3” to increase the number of points
        os.system(f"openMVG_main_GeometricFilter -i {matches_path}/sfm_data_gtpose.json -m {matches_path}/matches.putative.bin -g a -o {matches_path}/matches.f.bin")
        os.system(f"openMVG_main_ComputeStructureFromKnownPoses -i {matches_path}/sfm_data_gtpose.json -m {matches_path} -o {recon_path}/sfm_data_structure.bin -d -f {matches_path}/matches.putative.bin")
        os.system(f"openMVG_main_ConvertSfM_DataFormat -i {recon_path}/sfm_data_structure.bin -o {recon_path}/sfm_data.json")
        os.system(f"openMVG_main_ConvertSfM_DataFormat -i {recon_path}/sfm_data_structure.bin -o {recon_path}/{scene_name}.ply -S")
        os.system(f"openMVG_main_ComputeSfM_DataColor -i {recon_path}/sfm_data_structure.bin -o {recon_path}/{scene_name}_color.ply")
